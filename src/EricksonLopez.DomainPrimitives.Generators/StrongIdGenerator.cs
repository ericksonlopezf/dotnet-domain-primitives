...
internal sealed class StrongIdGenerator : IIncrementalGenerator
{
    ...
    internal static StrongIdTypeInfo? ExtractTypeInfo(
        ...
        CancellationToken ct)
    {
        ...
        if (backingFullName != "int" && backingFullName != "long" && backingFullName != "string" && backingFullName != "Guid" && backingFullName != "System.Guid")
            return null;
    ...
    internal static string GenerateStrongId(StrongIdTypeInfo info)
    {
        ...
        GeneratorHelpers.GenerateJsonConverter(sb, info.TypeName, info.BackingTypeName, rejectsNull: info.RejectEmpty);
    ...
    private static void GenerateParsing(SourceBuilder sb, StrongIdTypeInfo info)
    {
        ...
        if (info.IsGuidBacked)
        {
            ...
            sb.OpenBrace();
            sb.AppendLine($"return TryCreate(parsed, out result, out _);");
            sb.CloseBrace();
        }
        else if (info.IsIntegerBacked)
        {
        ...
        else // string-backed
        {
            ...
            sb.OpenBrace();
            sb.AppendLine($"return TryCreate(s, out result, out _);");
            sb.CloseBrace();
        }
        sb.AppendLine($"result = default;");
        sb.AppendLine("return false;");
    ...
    private static void GenerateOperators(SourceBuilder sb, StrongIdTypeInfo info)
    {
        ...
        sb.AppendLine($"    id.IsDefault ? throw new InvalidOperationException(\"Cannot convert a default {info.TypeName} to {info.BackingTypeName}. Check IsDefault before casting.\") : id._value;");
        sb.AppendLine();
