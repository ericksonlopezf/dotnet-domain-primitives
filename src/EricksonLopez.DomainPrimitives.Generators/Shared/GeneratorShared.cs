...
internal static class GeneratorShared
{
    ...
    /// <summary>
    /// Syntax-only predicate that matches readonly record structs decorated with any of the given
    /// trigger attribute names. Kept for generators where <c>ForAttributeWithMetadataName</c>
    /// cannot easily merge multiple pipelines (e.g., SmartEnum with user-defined static members).
    /// </summary>
    public static bool IsCandidateRecordStruct(SyntaxNode node, string[] triggerAttributeNames)
    {
        if (node is not RecordDeclarationSyntax rds)
            return false;
        if (!rds.ClassOrStructKeyword.IsKind(SyntaxKind.StructKeyword))
            return false;
        if (!rds.Modifiers.Any(SyntaxKind.ReadOnlyKeyword))
            return false;

        foreach (var attributeList in rds.AttributeLists)
        {
            foreach (var attribute in attributeList.Attributes)
            {
                ...
                string baseName = bracketIndex >= 0 ? name.Substring(0, bracketIndex) : name;

                int dotIndex = baseName.LastIndexOf('.');
                string simpleName = dotIndex >= 0 ? baseName.Substring(dotIndex + 1) : baseName;

                foreach (var trigger in triggerAttributeNames)
                {
                    if (simpleName == trigger || simpleName == trigger + "Attribute")
                        return true;
                }
            }
        }
