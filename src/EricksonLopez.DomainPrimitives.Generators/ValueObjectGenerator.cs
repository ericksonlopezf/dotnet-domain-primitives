...
internal sealed class ValueObjectGenerator : IIncrementalGenerator
{
        ...
        CancellationToken ct)
    {
        ...
        foreach (var member in typeSymbol.GetMembers())
        {
                ...
                propSymbol.DeclaredAccessibility == Accessibility.Public)
            {
                ...
                var isValueType = propSymbol.Type.IsValueType;
                var isNullable = propSymbol.NullableAnnotation == NullableAnnotation.Annotated ||
                                 (propSymbol.Type is INamedTypeSymbol nts && nts.OriginalDefinition.SpecialType == SpecialType.System_Nullable_T);
    ...
    internal static string GenerateValueObject(ValueObjectTypeInfo info)
    {
        ...
        // ── Backing state & IsDefault ──────────────────────────────────────────
        if (info.Properties.Length == 0)
        {
            sb.AppendLine("/// <summary>Returns true if this instance was created via default(T) rather than via Create().</summary>");
            sb.AppendLine("public bool IsDefault => true;");
            sb.AppendLine();
        }
        else
        {
            sb.AppendLine("private bool _isInitialized { get; init; }");
            sb.AppendLine();
            sb.AppendLine("/// <summary>Returns true if this instance was created via default(T) rather than via Create().</summary>");
    ...
    private static void GenerateFactoryMethods(SourceBuilder sb, ValueObjectTypeInfo info)
    {
        ...
        sb.AppendLine($"var instance = new {info.TypeName} {{ {assignments}, _isInitialized = true }};");
        ...
        if (info.Properties.Length > 0)
        {
            var deconstructParams = string.Join(", ", info.Properties.Values.Select(p => $"out {p.TypeName} {p.CamelCaseName}"));
            sb.AppendLine("/// <summary>Deconstructs the value object into its constituent components.</summary>");
            sb.AppendLine($"public void Deconstruct({deconstructParams})");
            sb.OpenBrace();
            foreach (var p in info.Properties.Values)
            {
                sb.AppendLine($"{p.CamelCaseName} = {p.Name};");
            }
            sb.CloseBrace();
            sb.AppendLine();
        }
    ...
    private static void GenerateJsonConverter(SourceBuilder sb, ValueObjectTypeInfo info)
    {
        ...
        foreach (var p in info.Properties.Values)
        {
            var declType = p.TypeName.EndsWith("?", global::System.StringComparison.Ordinal) ? p.TypeName : $"{p.TypeName}?";
            sb.AppendLine($"{declType} {p.CamelCaseName}Value = default;");
            sb.AppendLine($"bool {p.CamelCaseName}Set = false;");
        }
        ...
        foreach (var p in info.Properties.Values)
        {
            ...
            sb.OpenBrace();
            sb.AppendLine($"{p.CamelCaseName}Value = global::System.Text.Json.JsonSerializer.Deserialize<{declType}>(ref reader, options);");
            sb.AppendLine($"{p.CamelCaseName}Set = true;");
            sb.CloseBrace();
        }
        if (!first)
        {
        ...
        foreach (var p in info.Properties.Values)
        {
            if (!p.IsNullable)
            {
                sb.AppendLine($"if (!{p.CamelCaseName}Set || {p.CamelCaseName}Value is null)");
                sb.AppendLine($"    throw new global::System.Text.Json.JsonException(\"Required property '{p.Name}' was missing or null in JSON object for {info.TypeName}.\");");
            }
        }
        sb.AppendLine();
        ...
        if (info.Properties.Length > 0)
        {
            var args = string.Join(", ", info.Properties.Values.Select(p =>
                p.IsValueType && !p.IsNullable
                    ? $"{p.CamelCaseName}Value.GetValueOrDefault()"
                    : $"{p.CamelCaseName}Value!"));
