...
internal sealed class NumericPrimitiveGenerator : IIncrementalGenerator
{
    ...
    private static void GenerateValidation(SourceBuilder sb, NumericPrimitiveTypeInfo info)
    {
        ...
        if (info.BackingTypeName == "double")
        {
            sb.AppendLine("if (double.IsNaN(value) || double.IsInfinity(value))");
            sb.IncreaseIndent();
            sb.AppendLine($"return new global::EricksonLopez.DomainPrimitives.Validation.PrimitiveError(\"RANGE\", $\"{info.TypeName} cannot be NaN or Infinity.\");");
            sb.DecreaseIndent();
        }
        else if (info.BackingTypeName == "float")
        {
        ...
        else if (info.RangeMax.HasValue && info.RangeMax.Value != double.MaxValue)
        {
            var op = info.RangeMaxExclusive ? ">=" : ">";
            var msg = info.RangeMaxExclusive ? "less than" : "at most";
            // HIGH-05: Guard against decimal overflow. double.MaxValue (~1.8e308) is not representable
            // as decimal (max ~7.92e28). If the user specified a Max value that overflows decimal,
            // skip the upper-bound check entirely — no valid decimal value can ever exceed it.
            const double decimalMaxAsDouble = 7.922816251426434e+28; // (double)decimal.MaxValue
            if (info.BackingTypeName == "decimal" && info.RangeMax.Value > decimalMaxAsDouble)
            {
                // The bound exceeds decimal representability — no check needed; all decimal values pass.
            }
            else
            {
        ...
        {
            if (info.BackingTypeName == "decimal")
            {
                sb.AppendLine($"if (Math.Round(value, {info.Scale.Value}) != value)");
            }
            else
            {
    ...
    private static void GenerateOperators(SourceBuilder sb, NumericPrimitiveTypeInfo info)
    {
        ...
        sb.AppendLine($"/// <summary>Explicitly converts a <see cref=\"{info.TypeName}\"/> to its underlying <c>{info.BackingTypeName}</c> value.</summary>");
        sb.AppendLine($"/// <exception cref=\"InvalidOperationException\">Thrown when the instance is the default (uninitialized) value. Always check <c>IsDefault</c> before casting.</exception>");
        sb.AppendLine("[MethodImpl(MethodImplOptions.AggressiveInlining)]");
        ...
        sb.AppendLine($"/// <summary>Explicitly converts a raw <c>{info.BackingTypeName}</c> to a <see cref=\"{info.TypeName}\"/>. Equivalent to calling <c>Create(value)</c> and enforces all validation rules.</summary>");
        sb.AppendLine($"/// <exception cref=\"DomainPrimitiveValidationException\">Thrown when the raw value fails validation.</exception>");
        sb.AppendLine($"public static explicit operator {info.TypeName}({info.BackingTypeName} value) => Create(value);");
        sb.AppendLine();

        if (info.AllowAddition)
        {
            sb.AppendLine($"public static {info.TypeName} operator +({info.TypeName} left, {info.TypeName} right) => Create(checked(({info.BackingTypeName})(left.Value + right.Value)));");
            sb.AppendLine();
        }
        ...
        {
            sb.AppendLine($"public static {info.TypeName} operator *({info.TypeName} left, {info.BackingTypeName} right) => Create(checked(({info.BackingTypeName})(left.Value * right)));");
            sb.AppendLine($"public static {info.TypeName} operator *({info.BackingTypeName} left, {info.TypeName} right) => Create(checked(({info.BackingTypeName})(left * right.Value)));");
            sb.AppendLine();
        }
        ...
        {
            var isIntegral = info.BackingTypeName is "int" or "long" or "short" or "byte" or "sbyte" or "uint" or "ulong" or "ushort";
            var negExpr = isIntegral ? $"checked(({info.BackingTypeName})(-value.Value))" : $"({info.BackingTypeName})(-value.Value)";
            sb.AppendLine($"public static {info.TypeName} operator -({info.TypeName} value) => Create({negExpr});");
            sb.AppendLine();
        }
    }
}
