...
internal sealed class DatePrimitiveGenerator : IIncrementalGenerator
{
        ...
        CancellationToken ct)
    {
        ...
        else if (domainShortcut == "FiscalYear")
        {
            kind = "DateOnly";
            minYear = 1900;
            foreach (var attr in attributes)
            {
                if (attr.AttributeClass?.Name == "FiscalYearAttribute")
                {
                    foreach (var named in attr.NamedArguments)
                    {
                        if (named.Key == "MinYear" && named.Value.Value is int my) minYear = my;
                    }
                }
            }
        }
    ...
    internal static string GenerateDatePrimitive(DatePrimitiveTypeInfo info)
    {
        ...
        GeneratorHelpers.GenerateJsonConverter(sb, info.TypeName, info.BackingTypeName, rejectsNull: true);
        sb.CloseBrace();
    ...
    private static void GenerateValidation(SourceBuilder sb, DatePrimitiveTypeInfo info)
    {
        ...
        sb.AppendLine("#if NET8_0_OR_GREATER");
        sb.AppendLine("[MethodImpl(MethodImplOptions.AggressiveInlining)]");
        sb.AppendLine($"public static void Validate({info.BackingTypeName} value) => Validate(value, null);");
        sb.AppendLine();
        sb.AppendLine("[MethodImpl(MethodImplOptions.AggressiveInlining)]");
        sb.AppendLine($"public static void Validate({info.BackingTypeName} value, global::System.TimeProvider? timeProvider)");
        sb.OpenBrace();
        sb.AppendLine("var error = TryValidate(value, timeProvider);");
        if (!string.IsNullOrEmpty(info.CustomExceptionType))
            sb.AppendLine($"if (error.IsError) throw new {info.CustomExceptionType}(error.Message);");
        else
            sb.AppendLine("if (error.IsError) throw new DomainPrimitiveValidationException(error);");
        sb.CloseBrace();
        sb.AppendLine();

        sb.AppendLine("[MethodImpl(MethodImplOptions.AggressiveInlining)]");
        sb.AppendLine($"private static global::EricksonLopez.DomainPrimitives.Validation.PrimitiveError TryValidate({info.BackingTypeName} value, global::System.TimeProvider? timeProvider = null)");
        sb.OpenBrace();

        if (info.PastOnly || info.FutureOnly || info.MaxAge.HasValue)
        {
            sb.AppendLine("timeProvider ??= global::System.TimeProvider.System;");
            string nowExpr = info.Kind switch
            {
                "DateOnly" => "DateOnly.FromDateTime(timeProvider.GetUtcNow().UtcDateTime)",
                "TimeOnly" => "TimeOnly.FromDateTime(timeProvider.GetUtcNow().UtcDateTime)",
                "DateTimeOffset" => "timeProvider.GetUtcNow()",
        ...
        if (info.DomainShortcut == "FiscalYear" && info.MinYear.HasValue)
        {
            sb.AppendLine($"if (value.Year < {info.MinYear.Value})");
            sb.IncreaseIndent();
            sb.AppendLine($"return new global::EricksonLopez.DomainPrimitives.Validation.PrimitiveError(\"TEMPORAL\", $\"{info.TypeName} year must be >= {info.MinYear.Value}.\");");
            sb.DecreaseIndent();
        }
        ...
        sb.AppendLine("#else");
        sb.AppendLine("[MethodImpl(MethodImplOptions.AggressiveInlining)]");
        sb.AppendLine($"public static void Validate({info.BackingTypeName} value)");
        sb.OpenBrace();
    ...
    private static void GenerateFactoryMethods(SourceBuilder sb, DatePrimitiveTypeInfo info)
    {
        ...
        sb.AppendLine("#if NET8_0_OR_GREATER");
        sb.AppendLine("[MethodImpl(MethodImplOptions.AggressiveInlining)]");
        sb.AppendLine($"public static {info.TypeName} Create({info.BackingTypeName} value) => Create(value, null);");
        sb.AppendLine();
        sb.AppendLine("[MethodImpl(MethodImplOptions.AggressiveInlining)]");
        sb.AppendLine($"public static {info.TypeName} Create({info.BackingTypeName} value, global::System.TimeProvider? timeProvider)");
        sb.OpenBrace();
        if (info.Kind == "DateTime" || info.BackingTypeName is "DateTime" or "System.DateTime")
        {
            sb.AppendLine("value = value.Kind switch");
            sb.OpenBrace();
            sb.AppendLine("DateTimeKind.Unspecified => DateTime.SpecifyKind(value, DateTimeKind.Utc),");
            sb.AppendLine("DateTimeKind.Local => value.ToUniversalTime(),");
            sb.AppendLine("_ => value");
            sb.CloseBrace();
            sb.AppendLine(";");
        }
        sb.AppendLine("Validate(value, timeProvider);");
        ...
        sb.AppendLine("[MethodImpl(MethodImplOptions.AggressiveInlining)]");
        sb.AppendLine($"public static bool TryCreate({info.BackingTypeName} value, out {info.TypeName} result, out global::EricksonLopez.DomainPrimitives.Validation.PrimitiveError validationError) => TryCreate(value, null, out result, out validationError);");
        sb.AppendLine();

        sb.AppendLine("/// <summary>Tries to create a valid instance using a custom TimeProvider. Returns a boolean indicating success.</summary>");
        sb.AppendLine("[MethodImpl(MethodImplOptions.AggressiveInlining)]");
        sb.AppendLine($"public static bool TryCreate({info.BackingTypeName} value, global::System.TimeProvider? timeProvider, out {info.TypeName} result, out global::EricksonLopez.DomainPrimitives.Validation.PrimitiveError validationError)");
        sb.OpenBrace();
        if (info.Kind == "DateTime" || info.BackingTypeName is "DateTime" or "System.DateTime")
        {
            sb.AppendLine("value = value.Kind switch");
        ...
        sb.AppendLine("validationError = TryValidate(value, timeProvider);");
    ...
    private static void GenerateFormatting(SourceBuilder sb, DatePrimitiveTypeInfo info)
    {
        ...
        sb.AppendLine();
        sb.AppendLine("public override string ToString() => IsDefault ? string.Empty : _value.ToString();");
        sb.AppendLine();

        sb.AppendLine("public string ToString(string? format, IFormatProvider? formatProvider)");
        sb.AppendLine("    => IsDefault ? string.Empty : _value.ToString(format, formatProvider);");
        sb.AppendLine();
    ...
    private static void GenerateOperators(SourceBuilder sb, DatePrimitiveTypeInfo info)
    {
        ...
        sb.AppendLine("[MethodImpl(MethodImplOptions.AggressiveInlining)]");
        sb.AppendLine($"public static explicit operator {info.BackingTypeName}({info.TypeName} value) =>");
        sb.AppendLine($"    value.IsDefault ? throw new InvalidOperationException(\"Cannot convert a default {info.TypeName} to {info.BackingTypeName}. Check IsDefault before casting.\") : value._value;");
        sb.AppendLine();
