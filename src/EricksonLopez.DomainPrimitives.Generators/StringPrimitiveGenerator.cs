...
internal sealed partial class StringPrimitiveGenerator : IIncrementalGenerator
{
        ...
        System.Collections.Immutable.ImmutableArray<string>.Builder allowedSchemes)
    {
        switch (shortcut)
        {
            ...
            case "IBAN":
                ...
                upperCase = true;
                if (regexPatterns.Count == 0)
                    // INFO-09: Validates structural format only (CC + check digits + BBAN charset/length).
                    // Does NOT verify the ISO 13616 mod-97 checksum algorithm.
                    // For full algorithmic IBAN validation, add a custom [Must] attribute or validator.
            ...
            case "ISBN":
                ...
                notEmpty = true;
                if (regexPatterns.Count == 0)
                    // INFO-09: Validates structural format for ISBN-10 and ISBN-13.
                    // Does NOT verify the ISBN check digit (Luhn/modulo-11 for ISBN-10, modulo-10 for ISBN-13).
                    // For algorithmic ISBN check digit validation, supplement with a custom validator.
            ...
            case "VIN":
                ...
                upperCase = true;
                if (regexPatterns.Count == 0)
                    // INFO-09: Validates 17-character VIN charset (excludes I, O, Q per ISO 3779).
                    // Does NOT verify the NHTSA/ISO position-9 transliteration check digit.
                    // For full VIN check digit validation, supplement with a custom validator.
    ...
    internal static string GenerateStringPrimitive(StringPrimitiveTypeInfo info)
    {
        ...
        GeneratorHelpers.GenerateJsonConverter(sb, info.TypeName, "string", rejectsNull: info.NotEmpty);
