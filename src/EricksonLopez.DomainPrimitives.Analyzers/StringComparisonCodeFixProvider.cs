...
public sealed class StringComparisonCodeFixProvider : CodeFixProvider
{
    ...
    public override async Task RegisterCodeFixesAsync(CodeFixContext context)
    {
        ...
        foreach (var diagnostic in context.Diagnostics)
        {
                ...
                if (expression != null)
                {
                    context.RegisterCodeFix(
                        CodeAction.Create(
                            ...
                            createChangedDocument: c => AppendValueAccessAsync(context.Document, expression, c),
            ...
            {
                var binaryExpr = token.Parent?.FirstAncestorOrSelf<BinaryExpressionSyntax>();
                if (binaryExpr != null)
                {
                    context.RegisterCodeFix(
                        CodeAction.Create(
    ...
    private static async Task<Document> AppendValueAccessAsync(Document document, ExpressionSyntax expression, CancellationToken cancellationToken)
    {
        var root = await document.GetSyntaxRootAsync(cancellationToken).ConfigureAwait(false);
        if (root == null) return document;

        ExpressionSyntax targetExpr = expression;
