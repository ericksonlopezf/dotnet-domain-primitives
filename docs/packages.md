...
`EricksonLopez.DomainPrimitives` is published as a family of 14 coordinated NuGet packages. All packages share the same version number and are released simultaneously.
...
These four packages form the foundation. Most consumers only need `EricksonLopez.DomainPrimitives`.
...
Each integration package contains runtime components and source generators (bundled as `analyzers/dotnet/cs` assets). The generators enable **auto-discovery** — no per-type annotation is required in your domain layer.
...
| `EricksonLopez.DomainPrimitives.NewtonsoftJson` | `net8.0; net9.0; net10.0` | Newtonsoft.Json (Json.NET) converters and contract resolver | ✅ |

> [!NOTE]
> **EFCore and OpenApi** depend on `EricksonLopez.DomainPrimitives.Abstractions` directly (not the meta-package) because they only need marker interfaces for type detection. **AspNetCore, Dapper, and Testing** depend on the `EricksonLopez.DomainPrimitives` meta-package.

> [!NOTE]
> **Mapping Note (Mapster / Mapperly):** For scalar primitives (`[StringPrimitive]`, `[StrongId]`, `[NumericPrimitive<T>]`), object mappers resolve the generated `explicit operator` automatically without requiring a dedicated mapping package. See [adr-017](adr/adr-017-mapster-integration-rationale.md).
...
| Package ID | TFMs | Role |
|-----------|------|------|
| `EricksonLopez.DomainPrimitives.Testing` | `net8.0; net9.0; net10.0` | Test helpers: assertions, builders (`PrimitiveBuilder<T,V>`), fake factories (`DomainPrimitiveFakeFactory`). Integrates with xUnit, AwesomeAssertions, NSubstitute, Verify.Xunit. |
...
All packages are published to [NuGet.org](https://www.nuget.org) under the `EricksonLopez` owner:
...
| `EricksonLopez.DomainPrimitives.NewtonsoftJson` | [nuget.org/packages/EricksonLopez.DomainPrimitives.NewtonsoftJson](https://www.nuget.org/packages/EricksonLopez.DomainPrimitives.NewtonsoftJson) |
