...
The source generators bundled in each package automatically detect your domain primitives and generate the appropriate converters (`ValueConverter` for EF Core, `TypeHandler` for Dapper, `ModelBinder` for ASP.NET Core, SchemaFilter for OpenAPI, and `JsonConverter` for Newtonsoft.Json).
