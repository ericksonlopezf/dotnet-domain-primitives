...
<#
.SYNOPSIS
    Automated zero-tolerance repository compliance & governance verifier for EricksonLopez.DomainPrimitives.
.DESCRIPTION
    Validates:
...
$missingHeaders = 0
$csFilesToCheck = Get-ChildItem -Path (Join-Path $RootDirectory "src"), (Join-Path $RootDirectory "tests") -Recurse -Filter "*.cs" -ErrorAction SilentlyContinue | Where-Object { $_.FullName -notmatch '[\\/](obj|bin|Snapshots)[\\/]' -and $_.Name -notmatch '\.verified\.cs$' }
foreach ($cs in $csFilesToCheck) {
...
$scriptAndYamlFiles = Get-ChildItem -Path (Join-Path $RootDirectory "scripts"), (Join-Path $RootDirectory ".github") -Recurse -Include "*.ps1", "*.yml", "*.yaml" -File -ErrorAction SilentlyContinue
foreach ($f in $scriptAndYamlFiles) {
    $firstLine = (Get-Content $f.FullName -TotalCount 1 -Encoding UTF8)
    if ($firstLine -notmatch "Copyright .* Erickson Lopez.* MIT License") {
        Write-Host "  ❌ Missing MIT header in $($f.FullName)" -ForegroundColor Red
        $violations++
        $missingHeaders++
    }
}
if ($missingHeaders -eq 0) { Write-Host "  ✅ All C#, script, and workflow files contain the required MIT copyright header." -ForegroundColor Green }

# 4. One Type Per File in src/ and tests/
Write-Host "`n[4/8] Checking 'One Type Per File' rule in src/ and tests/..." -ForegroundColor Yellow
$multiTypeFiles = 0
foreach ($cs in $csFilesToCheck) {
...
if ($multiTypeFiles -eq 0) { Write-Host "  ✅ Every production and test file satisfies the 'One Type Per File' invariant." -ForegroundColor Green }
...
foreach ($f in $allTrackedFiles) {
    ...
    for ($i = 0; $i -lt $lines.Count; $i++) {
        if ($lines[$i] -match "github\.com/ericksonlopez/dotnet-domain-primitives\b") {
...
if ($badLinks -eq 0) { Write-Host "  ✅ All GitHub URLs correctly target ericksonlopezf/dotnet-domain-primitives." -ForegroundColor Green }
...
# -----------------------------------------------------------------------------
# Documentation Relative Links Gate
# -----------------------------------------------------------------------------
Write-Host "`n[Gate: Markdown Links] Checking relative links across documentation..." -ForegroundColor Yellow
$mdFiles = Get-ChildItem -Path $targetRoot -Recurse -Filter "*.md" -File -ErrorAction SilentlyContinue | Where-Object { $_.FullName -notmatch '[\\/](obj|bin|\.git)[\\/]' }
$brokenLinks = 0
foreach ($md in $mdFiles) {
    ...
    foreach ($m in $linkMatches) {
        ...
        $targetPath = Join-Path $dir $pathOnly
        if (-not (Test-Path $targetPath)) {
            Write-Host "  ❌ Broken relative link in $($md.FullName): '$link'" -ForegroundColor Red
            if (Get-Variable -Name "violations" -Scope 0 -ErrorAction SilentlyContinue) { $violations++ }
            $brokenLinks++
        }
    }
}
if ($brokenLinks -eq 0) { Write-Host "  ✅ All relative documentation links resolve to valid filesystem targets." -ForegroundColor Green }
